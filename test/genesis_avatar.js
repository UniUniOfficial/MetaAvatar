...
contract("GenesisAvatar", function (accounts) {
  ...
  it("It should withdraw all the coin of the contract", async function () {
    ...
    let totalAmount = web3.utils.toBN(await web3.eth.getBalance(ga.address));
    let oldAmountOfAccount9 = web3.utils.toBN(await web3.eth.getBalance(account9));
    await ga.withdraw(account9, {from: owner})
    let newAmountOfAccount9 = web3.utils.toBN(await web3.eth.getBalance(account9));
